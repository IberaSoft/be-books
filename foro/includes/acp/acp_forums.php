	...
	function main($id, $mode)
	{
		...
		if ($row = $db->sql_fetchrow($result))
		{
			do
			{
				...
				{
					$folder_image = '<img src="assets/images/icon_folder_lock.gif" alt="' . $user->lang['LOCKED'] . '" />';
				}
				else
				{
					switch ($forum_type)
					{
						case FORUM_LINK:
							$folder_image = '<img src="assets/images/icon_folder_link.gif" alt="' . $user->lang['LINK'] . '" />';
						break;

						default:
							$folder_image = ($row['left_id'] + 1 != $row['right_id']) ? '<img src="assets/images/icon_subfolder.gif" alt="' . $user->lang['SUBFORUM'] . '" />' : '<img src="assets/images/icon_folder.gif" alt="' . $user->lang['FOLDER'] . '" />';
						break;
					}
				}
