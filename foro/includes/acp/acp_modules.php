	...
	function main($id, $mode)
	{
		...
		if ($row = $db->sql_fetchrow($result))
		{
			do
			{
				...
				{
					$module_image = '<img src="assets/images/icon_folder_lock.gif" alt="' . $user->lang['DEACTIVATED_MODULE'] .'" />';
				}
				else
				{
					$module_image = (!$row['module_basename'] || $row['left_id'] + 1 != $row['right_id']) ? '<img src="assets/images/icon_subfolder.gif" alt="' . $user->lang['CATEGORY'] . '" />' : '<img src="assets/images/icon_folder.gif" alt="' . $user->lang['MODULE'] . '" />';
				}
